...
class AuthService:
        ...
        if rows:
            ...
            with self.db.transaction() as conn:
                for row in rows:
                    ...
                    invitations[row["employee_id"]] = password
            self._save_bootstrap("employee-invitations.json", invitations)

    def _save_bootstrap(self, name, values):
        path = self.db.path.parent / name
    ...
    def reset_employee_password(self, employee_id: str, *, actor_role: str) -> str:
        ...
        password = secrets.token_urlsafe(12)
        with self.db.transaction() as conn:
            conn.execute("UPDATE employee_accounts SET password_hash=?,must_change=1,version=version+1 WHERE employee_id=?", (self._hash(password), employee_id))
    ...
    def initialize_defaults(self) -> None:
        ...
        with self.db.transaction() as conn:
            for role in missing:
                password = os.getenv("CAMERA_ADMIN_PASSWORD") if role == ADMIN else None
                if not password:
                    password = secrets.token_urlsafe(18)
                    self._save_bootstrap("bootstrap-accounts.json", {role: password})
                conn.execute(
                    """INSERT OR IGNORE INTO auth_settings(role, password_hash, updated_at)
                       VALUES (?, ?, ?)""",
                    (role, self._hash(password), now),
                )
