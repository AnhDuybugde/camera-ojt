...
_load_environment(BASE_DIR.parents[1] / ".env")
...
class Settings:
    ...
    camera_ojt_url: str = os.getenv(
        "CAMERA_OJT_URL", "http://127.0.0.1:8765"
    ).strip().rstrip("/")
    camera_ojt_supervisor_url: str = os.getenv(
        "CAMERA_OJT_SUPERVISOR_URL", "http://127.0.0.1:8766"
    ).strip().rstrip("/")
    camera_ojt_queue_path: Path = _local_path(
        "CAMERA_OJT_QUEUE_PATH", BASE_DIR.parents[1] / "output" / "queue.db")
    camera_ojt_person_map_path: Path = _local_path(
        "CAMERA_OJT_PERSON_MAP_PATH", BASE_DIR / "data" / "person_map.json")
    camera_ojt_bridge_enabled: bool = False
