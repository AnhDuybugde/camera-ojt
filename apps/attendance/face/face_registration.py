...
class FaceRegistrationService:
    ...
    def register_from_capture(
        ...
        self.db.save_embedding_samples(employee_id, [v.tolist() for v in collected],
            actor_role=self.actor_role, actor_employee_id=self.actor_employee_id)
        return embedding
    ...
    def replace_from_embeddings(self, employee_id: str, embeddings: list[np.ndarray]) -> None:
        ...
        self.db.save_embedding_samples(employee_id, [v.tolist() for v in embeddings],
            actor_role=self.actor_role, actor_employee_id=self.actor_employee_id)
