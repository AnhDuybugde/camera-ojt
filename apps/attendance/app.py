...
ALL_PAGES = (
    ...
    ("Hệ thống AI & Trợ lý", "AI System", "smart_toy", "ai_system"),
...
with st.sidebar:
    ...
    primary_keys = {"dashboard", "live", "registration", "history"}
    primary_menu = [item for item in menu if item[3] in primary_keys]
    secondary_menu = [item for item in menu if item[3] not in primary_keys]
    for label, _, icon, key in primary_menu:
        st.button(
    ...
    if secondary_menu:
        with st.expander("Quản trị & tiện ích", expanded=False, icon=":material/menu:"):
            for label, _, icon, key in secondary_menu:
                st.button(
                    label, key=f"nav_{key}", icon=f":material/{icon}:", width="stretch",
                    type="primary" if st.session_state.active_page == label else "tertiary",
                    on_click=_select_page, args=(label,),
...
camera_ojt_bridge = None  # Backend owns ingestion independently of browser sessions.
...
if page == "Dashboard":
    ...
    system_integration.render(role, camera_ojt_bridge)
else:
