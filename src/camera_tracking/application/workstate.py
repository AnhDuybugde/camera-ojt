...
"nguoi do dang lam gi" (UNKNOWN/WORKING/AWAY/AT_DOOR/OUT_OF_DOOR) va khong
...
class DisabledStream:
    """An intentionally disabled camera, distinct from a failed capture."""
...
def parse_args(profile: str = "imou") -> argparse.Namespace:
    ...
    parser.add_argument("--source-a", default="0" if profile == "webcam" else None,
                        help="Override camera A.")
    ...
    parser.add_argument("--out-after-s", type=float, default=60.0,
                        help="Deprecated, ignored (5-state mode: no POSSIBLY_OUT).")
    parser.add_argument("--return-stable-s", type=float, default=2.0,
                        help="Deprecated, ignored (5-state mode: no RETURNING).")
    ...
    parser.add_argument("--no-image-zones", action="store_true",
                        help="Tat rule zone R1/R2/R3 (quay ve workstation ROI / "
                        "presence-only cu). Mac dinh bat.")
    ...
    *,
    ...
    for row_id, kind, payload, _attempts in queue.peek(100):
        if kind == AIMIND_TICK_KIND:
            continue  # owned by the ai-mind-attendance bridge, not Supabase
...
def main(profile: str = "imou") -> None:
    ...
    if args.device == "cpu":
        config.identity = config.identity.model_copy(update={"reid_device": "cpu"})
        config.face = config.face.model_copy(update={"face_device": "cpu"})
        if args.model is None and str(config.detection.model_path).endswith(".engine"):
            args.model = str(Path(config.detection.model_path).with_suffix(".pt"))
    from uuid import uuid4
    from camera_tracking.workstate.door import DoorTransitions
    from camera_tracking.workstate.image_zones import point_in_polygon
    runtime_session = str(uuid4())
    door_transitions = DoorTransitions(config.door.dwell_s, config.door.max_gap_s)
    if profile == "webcam":
        config.voice = config.voice.model_copy(update={"backend": "local"})
        args.voice_trigger = False
        if args.face_channels is None:
            args.face_channels = "A"
    ...
    source_b = normalize_source(
        args.source_b or (imou_url(args.channel_b, args.subtype_b) if profile == "imou" else None)
    )
    single_channel = source_b is None
    if single_channel and profile == "imou":
    ...
    # Business theo channel: image zones R1/R2/R3 uu tien truoc, sau do
    # moi den workstation ROI + presence. Khong bao gio anh huong ID.
    # - Channel A: tam bbox trong R1 -> Away, ngoai R1 -> Working.
    # - Channel B: tam trong R2 -> Out of door, trong R3 -> At door,
    #   ngoai ca hai -> Away (R2 uu tien khi chong lan).
    # --no-image-zones quay ve workstation ROI / presence-only cu.
    ...
    img_zones: dict[str, list[tuple[float, float]]] = {}
    use_image_zones = not args.no_image_zones
    if use_image_zones:
        try:
            img_zones = load_channel_zones(_PROJECT_ROOT / "data" / "processed")
            print(f"Image zones: R1({len(img_zones.get('R1', []))}pts) "
                  f"R2({len(img_zones.get('R2', []))}pts) "
                  f"R3({len(img_zones.get('R3', []))}pts) "
                  f"(normalized 0..1, bbox-center rule)")
        except (OSError, ValueError) as error:
            print(f"Image zones unavailable ({error}), fallback ROI/presence.")
            img_zones = {}
            use_image_zones = False
    business_a = ChannelBusinessTracker(
        ...
        workstations=ws_zones if (use_roi and not use_image_zones) else [],
        ...
        move_ratio=move_ratio if not use_image_zones else 0.0,
        settle_ratio=ws_cfg.settle_ratio,
        zone_mode="a_r1" if use_image_zones else "none",
        zone_r1=list(img_zones.get("R1", [])),
        zone_r2=list(img_zones.get("R2", [])),
        zone_r3=list(img_zones.get("R3", [])),
    )
    business_b = ChannelBusinessTracker(
        ...
        out_after_s=args.out_after_s,
        return_stable_s=args.return_stable_s,
        # Channel B là camera cửa (transit): không dùng workstation ROI
        # của phòng A để khỏi gán nhầm Working.
        workstations=[],
        grace_s=ws_cfg.grace_s,
        dwell_s=ws_cfg.dwell_s,
        assign_dwell_s=ws_cfg.assign_dwell_s,
        ...
        move_ratio=0.0,
        settle_ratio=ws_cfg.settle_ratio,
        zone_mode="b_door" if use_image_zones else "none",
        zone_r1=list(img_zones.get("R1", [])),
        zone_r2=list(img_zones.get("R2", [])),
        zone_r3=list(img_zones.get("R3", [])),
    )
    ...
    if use_face:
        try:
            ...
            from camera_tracking.face.enrollment import refresh_enrollment
            enrollment_version = f"{face_cfg.model_pack}:insightface-0.7:aligned-v1:det{face_cfg.det_size}"
            employee_db = Path(os.getenv("CAMERA_DATABASE_PATH") or
                               _PROJECT_ROOT / "apps/attendance/data/database.db")
            enrollment_store = EmbeddingStore(employee_db)
            face_gallery = load_gallery(
                face_cfg.gallery_dir,
                face_embedder,
                face_cfg.name_map,
                face_cfg.employee_map,
                max_seed_prototypes=29,
                sample_store=enrollment_store,
                model_version=enrollment_version,
            )
            refresh_enrollment(face_gallery, enrollment_store, enrollment_version, employee_db)
    ...
    if not args.no_stream and args.stream_port:
        ...
        def ask_assistant(payload: dict) -> dict:
            question = " ".join(str(payload.get("question", "")).split())
            if not 2 <= len(question) <= 500:
                return {"ok": False, "status": 400,
                        "message": "Câu hỏi cần từ 2 đến 500 ký tự."}
            try:
                from camera_tracking.voice.query import think
                model = (os.getenv("GEMINI_MODEL", "").strip()
                         or "gemini-3.6-flash")
                answer, tool_s, tools = think(
                    question, model, 300, search_results=5)
                ...
                return {"ok": False, "status": 502,
                        "message": "Trợ lý tạm không khả dụng. Kiểm tra cấu hình Gemini/Tavily ở backend."}

        streamer.set_attendance_actions(pending_attendance, send_attendance)
        streamer.set_enrollment_action(register_person)
        streamer.set_assistant_action(ask_assistant)
        if stream_on:
    ...
    cap_b = (open_capture(source_b, attempts=args.open_attempts, camera_name="B")
             if not single_channel else None)
    opened_b = cap_b is not None and cap_b.isOpened()
    if not opened_b:
        print(f"Không mở được camera B: {source_label(source_b)}")
    if not opened_a and not opened_b:
        stream_a.close()
        raise SystemExit(1)
    stream_b = DisabledStream() if single_channel else low_latency_capture(ResilientCapture(
        source_b,
        "B",
        cap_b,
        args.open_attempts,
    ...
    if args.halinh:
        halinh_script = (args.halinh_script
                         or str(_PROJECT_ROOT / "scripts" / (
                             "halinh_assistant_local.py" if profile == "webcam"
                             else "halinh_assistant.py")))
    ...
    def _handle_one_face(*, channel: str, frame, track, crop, det, match,
        ...
        if match.is_known and match.person is not None:
            ...
            if (
                config.door.legacy_face_attendance
                and ticked is not None
                and day_cache.attendance_should_write(day_str, ticked.person_id)
                    ...
                    # ai-mind-attendance bridge (phase 1): queue a check-in
                    # tick; the bridge maps person_id -> employee_id and
                    # calls AttendanceService.record() (rules stay there).
                    _store_or_queue(write_queue, supabase, AIMIND_TICK_KIND,
                                    build_checkin_payload(
                                        day=day_str, person_id=ticked.person_id,
        ...
        for track in sorted(confirmed_tracks, key=_face_priority):
            ...
            try:
                ...
                accepted = face_worker.submit(_FJ(
                    ...
                    generation=getattr(manager.identities.get(gid), "generation", 0),
                ))
                if accepted:
    ...
    def _drain_face_results(*, frame_a, frame_b, face_marks_a: list,
        ...
        with metrics.measure("face_apply"):
            for res in results:
                ...
                current_tracks = last_tracks_a if res.channel == "A" else last_tracks_b
                current_track = next((t for t in current_tracks if t.track_id == res.gid), None)
                record = manager.identities.get(res.gid)
                # A completed inference is evidence about its original observation,
                # never permission to bind a disappeared, reassigned or occluded body.
                if (marks is None or current_track is None or record is None
                        or record.occluded
                        or record.generation != res.generation
                        or current_track.local_track_id != res.track.local_track_id
                        or now_s - res.now_s > 2.0):
                    continue
                try:
                    _handle_one_face(
    ...
    def write_runtime_status() -> None:
        ...
        payload = {
            ...
            "camera_b": _stream_state(stream_b, not single_channel),
        ...
        while True:
            ...
            if process_frame:
                ...
                if ret_a and frame_a is not None:
                    ...
                    # Image zones work in normalized image space: always feed
                    # bbox centers, never floor-meter foot points.
                    floor_a = (
                        _track_norm_centers(
                            confirmed_a,
                            config.camera.frame_width,
                        ...
                        if (use_image_zones or not use_roi)
                        else _track_floor_points(confirmed_a, projector)
                    )
                ...
                if ret_b and frame_b is not None:
                    ...
                    floor_b = (
                        ...
                        if (use_image_zones or not use_roi)
                        else _track_floor_points(confirmed_b, projector)
                    )
                    try:
            ...
            if process_frame:
                ...
                for gid, record in manager.identities.items():
                    if record.occluded or record.employee_id is None:
                        gid_to_person.pop(gid, None)
                        gid_to_display.pop(gid, None)
                        gid_to_face_rankings.pop(gid, None)
                        gid_face_ranking_at.pop(gid, None)
                if config.door.enabled and use_image_zones and img_zones:
                    door_tracks = last_tracks_b if config.door.channel == "B" else last_tracks_a
                    door_frame = frame_b if config.door.channel == "B" else frame_a
                    door_ret = ret_b if config.door.channel == "B" else ret_a
                    if door_ret and door_frame is not None:
                        height, width = door_frame.shape[:2]
                        for track in door_tracks:
                            record = manager.identities.get(track.track_id)
                            center = ((track.bbox.x1 + track.bbox.x2) / (2 * width),
                                      (track.bbox.y1 + track.bbox.y2) / (2 * height))
                            zone = ("outside" if point_in_polygon(center, img_zones["R2"])
                                    else "door" if point_in_polygon(center, img_zones["R3"])
                                    else "inside")
                            direction = door_transitions.update(
                                (config.door.channel, track.track_id), zone, now_s,
                                uncertain=record is None or record.occluded)
                            if direction:
                                employee = record.employee_id if record else None
                                # Only a fresh, unambiguous face anchor can name a passage.
                                if (record is None or record.face_anchor_s is None
                                        or now_s - record.face_anchor_s > 3.0):
                                    employee = None
                                write_queue.push(AIMIND_TICK_KIND, {
                                    "event_id": str(uuid4()), "session_id": runtime_session,
                                    "camera_id": config.door.channel, "tick": direction,
                                    "direction_confirmed": True, "at": wall_iso,
                                    "person_id": employee or "", "global_id": track.track_id,
                                    "evidence": {"zone": zone, "global_id": track.track_id},
                                })
                    door_transitions.expire(now_s)
                _snapshot_new_gids(
                ...
                for gid, st in room_status_now.items():
                    ...
                    if log_event and established and gid_to_person.get(wgid) and (
                        ...
                        # ai-mind-attendance bridge: leaving (R2 out or the
                        # 15-min absent timer) is a check-out attempt;
                        # record() enforces CHECKOUT_MIN_MINUTES itself.
                        if (config.door.legacy_face_attendance
                                and st.just_left_office and gid_to_person.get(wgid)):
                            _store_or_queue(
                                write_queue, supabase, AIMIND_TICK_KIND,
                                build_checkout_payload(
                                    day=day_str,
                                    person_id=str(gid_to_person[wgid]),
                                    display_name=str(gid_to_display.get(
                                        wgid, gid_to_person[wgid])),
                                    global_id=wgid, wall_iso=wall_iso,
                                ))
                        _note_event(
                            event="LEAVE_OFFICE" if st.just_left_office else "RETURN",
                            global_id=wgid,
            ...
            if stream_on:
                ...
                if process_frame:
                    ...
                    streamer.set_status({
                        ...
                        "services": {
                            "voice_enabled": bool(args.halinh),
                            "gemini_configured": bool(os.getenv(
                                "GEMINI_API_KEY", "").strip()),
                            "search_provider": (
                                "tavily" if os.getenv("TAVILY_API_KEY", "").strip()
                                else "fallback"
                            ),
                        },
                    })
            ...
            if process_frame and now_s - last_runtime_write_s >= 2.0:
                ...
                if use_face and face_gallery is not None:
                    try:
                        with face_model_lock:
                            refresh_enrollment(face_gallery, enrollment_store,
                                               enrollment_version, employee_db)
                            active_employees = {p.employee_id or p.person_id for p in face_gallery.people}
                            for gid, identity in manager.identities.items():
                                if identity.employee_id and identity.employee_id not in active_employees:
                                    manager.unbind_employee(gid)
                                    gid_to_person.pop(gid, None)
                                    gid_to_display.pop(gid, None)
                    except (OSError, ValueError):
                        pass
