"""Tracking Global ID 1 webcam (ban LOCAL) + voice Ha Linh luon bat.

Giong het run_workstate.py nhung:
  * Camera A = webcam may tinh (mac dinh 0); kenh B tat (single-channel).
    Muon 2 kenh nhu ban goc thi truyen --source-b (file video/RTSP).
  * Vay tay 5 ngon / gio tay + face-trigger VAN CHAO nhu ban goc, nhung
    phat bang loa laptop (backend local) thay vi loa camera.
  * voice-trigger (mic RTSP) tat; thay bang voice Ha Linh
    (scripts/halinh_assistant_local.py) chay subprocess rieng,
    tu restart khi crash, dung sach khi pipeline dung.
  * Model YOLO/ByteTrack/ReID/Face/MediaPipe giu nguyen nhu kenh A ban goc.

Chay:
  python scripts/run_workstate_local.py --display
  python scripts/run_workstate_local.py --source-a 0 --display
  python scripts/run_workstate_local.py --no-halinh   # tat voice Ha Linh
...
class DisabledStream:
    """Kenh B tat (single-channel): read() luon False, loop tu bo qua."""
...
def supervise_halinh(
    ...
    """Chay voice Ha Linh subprocess, tu restart khi crash (luon bat).

    Dung khi pipeline dung (stop_event) hoac het luot restart. Log voice
    giu tag [HaLinh-local] rieng de phan biet voi log tracking.
    """
    restarts = 0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tracking ID webcam local + voice Ha Linh (YOLO + ByteTrack + Re-ID).")
    ...
    parser.add_argument("--source-a", default="0", help="Nguon camera A (mac dinh webcam 0).")
    parser.add_argument("--source-b", default=None,
                        help="Truyen de bat kenh B (file video/RTSP). Bo trong = single-channel.")
    parser.add_argument("--halinh", dest="halinh",
                        action=argparse.BooleanOptionalAction, default=True,
                        help="Luon bat voice Ha Linh (mic+loa laptop) song song tracking. "
                             "Mac dinh bat, tat bang --no-halinh.")
    parser.add_argument("--halinh-script", default=None,
                        help="Duong dan voice script (mac dinh scripts/halinh_assistant_local.py).")
    ...
    parser.add_argument("--halinh-cmd-model", default="medium",
                        help="Model STT vong nghe lenh cua voice (medium/small). "
                             "Chay chung voi tracking thi small nhe CPU hon.")
    parser.add_argument(
        "--model",
...
def main() -> None:
    ...
    try:
        config.voice.backend = "local"  # type: ignore[assignment]
    except (ValueError, TypeError):
        config.voice = config.voice.model_copy(update={"backend": "local"})
    # voice-trigger can mic camera RTSP (webcam khong co mic) nen tat han;
    # voice Ha Linh (mic+loa laptop) thay the.
    ...
    source_b = normalize_source(args.source_b) if args.source_b else None
    single_channel = source_b is None
    if single_channel:
        print("Single-channel: chi dung webcam A (+ voice Ha Linh). "
              "Muon 2 kenh thi truyen --source-b.")
    ...
    print(
        ...
        f"Camera B: {'tat (single-channel)' if single_channel else source_label(source_b)}"
    )
    if str(model_path).endswith(".engine"):
        print("YOLO: TensorRT engine (FP16, dynamic batch) — ky vong ~150+ fps tren GPU.")
    else:
    ...
    # Voice Ha Linh luon bat song song tracking (subprocess rieng, tu
    # restart khi crash). Tat bang --no-halinh.
    halinh_stop: threading.Event | None = None
    halinh_thread: threading.Thread | None = None
    if args.halinh:
        halinh_script = (args.halinh_script
                         or str(_PROJECT_ROOT / "scripts" / "halinh_assistant_local.py"))
    ...
    def write_runtime_status() -> None:
        """Ghi trang thai runtime cho voice Ha Linh doc (2s/lan, atomic).
    ...
    try:
        while True:
            ...
            # Cau runtime cho voice Ha Linh (2s/lan): dem nguoi, camera,
            # fps, diem danh, su kien -> tool tra loi so THAT.
