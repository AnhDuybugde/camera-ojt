...
from frontend.src.components.components import page_header, section_header
...
def render(role: str) -> None:
    ...
    groups = (
        ("Backend camera-ojt", "bi-cpu", {
            "Realtime API": settings.camera_ojt_url or "Tắt",
            "Supervisor": settings.camera_ojt_supervisor_url or "Tắt",
            "Bridge SQLite": "Tự động" if settings.camera_ojt_bridge_enabled else "Tắt",
        }),
    ...
    for column, group in zip(st.columns(4), groups):
        with column:
