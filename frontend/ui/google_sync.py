...
def render(db: Database, service: SyncService, role: str) -> None:
    ...
    if st.button("Đồng bộ ngay", type="primary", icon=":material/sync:"):
        with st.spinner("Đang đồng bộ dữ liệu…"):
            # A long-running Streamlit process may still hold the pre-auth
            # SyncService instance. The page itself is already ADMIN-gated;
            # use its legacy call shape until the process is restarted.
            if "actor_role" in inspect.signature(service.sync_pending).parameters:
