...
class ConfigTest(TestCase):
    def test_load_config(self) -> None:
        ...
        self.assertEqual(config.identity.match_threshold, 0.55)
        self.assertEqual(config.identity.min_appearance_similarity, 0.50)
        self.assertEqual(config.identity.named_appearance_floor, 0.60)
        self.assertEqual(config.identity.active_duplicate_similarity, 0.70)
        self.assertEqual(config.identity.gallery_refresh_steps, 5)
        self.assertEqual(config.identity.reid_backend, "face")
        self.assertEqual(config.identity.face_min_px, 40)
        self.assertEqual(config.face.match_threshold, 0.55)
